-- `modify_order`: a CNC sell cannot be raised beyond the holding.
--
-- The Phase 4 checkpoint review found this. `place_order` rejects a SELL/CNC
-- order with NO_HOLDING the moment the quantity exceeds the holding, so the user
-- is told now rather than at a fill days away. `modify_order` validated the
-- quantity's sign and the limit price against the order type, and nothing else —
-- and `reserve_margin` reserves nothing for a CNC sell, so there was no second
-- gate behind it either.
--
-- The result: a resting CNC sell of 10 could be modified to 10,000 against a
-- 10-share holding and accepted silently, only to be rejected by
-- `execute_order`'s own re-check whenever it finally crossed. The same terms
-- refused at placement were accepted at modify — one path telling the user
-- something the other would not.
--
-- `execute_order` still re-checks under the lock and that stays load-bearing: a
-- holding can be sold from under a resting limit order between the modify and
-- the fill. This makes the two entry points agree, it does not replace the
-- guard at the end.
--
-- Everything else is the definition from 20260903130000_modify_order.sql,
