-- The capped-loss path credited the shortfall on top of a partial debit.
--
-- §6 caps the cash effect of a cover the account cannot fund: the position still
-- closes, `available_cash` floors at zero, and the uncovered remainder is booked
-- as a SIMULATION_ADJUSTMENT so `available_cash = Σ ledger` still holds.
--
-- The first implementation debited the *capped* amount as REALISED_PNL and then
-- credited the difference as well, so the two rows netted to roughly the cap
-- again and the account ended with cash it should not have had — 2916.96 on the
-- worked case below, where the answer is 0.00.
--
-- Two things are corrected. The REALISED_PNL row now carries the loss **in
-- full**, which is what makes the adjustment mean "the part the account could
-- not cover". And the credit is written **first**: available_cash and
-- balance_after both carry non-deferrable CHECKs, so debiting the whole loss and
-- crediting the shortfall afterwards is refused with 23514 on the intermediate
-- row rather than landing on the same place.
--
-- Worked case, now asserted in `10-orders.sql`: a short of 100 at 100.00 on an
-- account holding 12100.00, covered at 250.00. Collateral 12005.06 comes back,
-- charges of 10.54 are paid, leaving 12083.04 against a 15000.00 loss. Cash
-- lands on exactly 0.00, SIMULATION_ADJUSTMENT carries 2916.96, and
-- `trades.realised_pnl` still records the true uncapped −15016.54.
