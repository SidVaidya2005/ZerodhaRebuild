-- `execute_order`: the opening leg is debited BEFORE the cover's loss is capped.
--
-- The Phase 4 checkpoint review found this. §6's cap deliberately floors
-- `available_cash` at exactly zero, and the opening leg's `BUY_DEBIT` was posted
-- immediately after it — so a BUY that both covered a short and opened a long,
-- where the cover's loss exceeded the balance, hit
-- `funds_available_cash_non_negative` and died with 23514. Neither that CHECK nor
-- `fund_ledger_balance_after_non_negative` is deferrable.
--
-- The whole fill aborted, which breaks §6's "the position still closes" and
-- architecture.md's "no position is ever stranded" — the loss cap exists so a
-- cover can always complete, and here it was the thing preventing one. Through
-- `place_order` the RPC aborted with no order row surviving, so the user saw a
-- fault rather than the normal return a business outcome is supposed to give;
-- through `match_open_orders` the order faulted on every tick forever, silently,
-- swallowed by the `exception when others` that counts `faulted`.
--
-- Section K only ever covered a PURE cover (`v_open_quantity = 0`), where no
-- `BUY_DEBIT` follows the cap at all, so the flip path had no coverage.
--
-- The fix is a reordering: post the opening leg first and cap against what is
-- left. §7 permits it outright — "within one event, the order of these rows is
-- not normative unless stated", and the cap carries no explicit *then*. It is
-- also the arithmetic the contract asks for: the cap must floor against the
-- balance remaining after every other cash movement of the fill, and the
-- purchase is one of them. The simulator absorbs what the account cannot cover
-- once the shares are paid for, not before.
--
-- The debit cannot itself be refused there. The solvency guard already required
-- `available_cash + blocked_margin >= open_quantity × price + charges`, and by
-- this point both the order's reservation and any position collateral have been
-- released back into cash, so the balance covers it with the collateral to spare.
--
-- Everything else is the definition from 20260905170000_closing_leg_never_rejected.sql,
