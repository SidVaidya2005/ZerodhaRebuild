-- `trades.traded_at` and the `orders` timestamps become `clock_timestamp()`.
--
-- The same defect F23 fixed on `fund_ledger.created_at`, in the two other places
-- it bites. `now()` is the transaction start time, so every row written by one
-- transaction ties, and "the most recent trade" is decided by whatever the
-- planner returns.
--
-- It is not hypothetical here. F28's matcher fills every crossed order in a
-- single run and F29's square-off closes every open MIS position in one — so
-- Reports, which orders by `traded_at desc`, would show a whole square-off in
-- arbitrary order, and any "latest fill" read would pick a row at random.
--
-- Found by F24's suite, which places several orders in one transaction and could
-- not ask for the trade it had just written.
alter table public.trades alter column traded_at set default clock_timestamp();
alter table public.orders alter column placed_at set default clock_timestamp();

comment on column public.trades.traded_at is
  'clock_timestamp(), not now(): the square-off job writes many trades in one transaction and Reports orders by this column, so rows within a transaction must be distinguishable.';

-- `executed_at` is assigned explicitly rather than defaulted, so the same
-- correction has to happen at each write site. Both functions are re-created
-- with `clock_timestamp()` in place of `now()`; nothing else about them changes.
...
begin
  ...
  perform public.release_margin(p_order_id);
  update public.orders set status = 'CANCELLED', executed_at = clock_timestamp() where id = p_order_id;
  return true;
end;
$$;
