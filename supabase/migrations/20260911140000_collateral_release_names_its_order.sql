-- The short cover's collateral release now names the order that caused it.
--
-- Found at the Phase 4 checkpoint by sweeping trading-contract.md §12.9 against
-- live data. That identity reads: "the sum of every ledger row referencing its
-- orders -- MARGIN_BLOCK, MARGIN_RELEASE, BUY_DEBIT, SELL_CREDIT, CHARGES,
-- REALISED_PNL, SIMULATION_ADJUSTMENT -- equals the net change in available_cash
-- across its lifetime. Every row is included."
--
-- It could not hold for any short cover. `recompute_position_collateral` took
-- (user_id, symbol, new_net_quantity) and so had no order id to stamp, and its
-- ledger insert omitted the column: the MARGIN_RELEASE that returns the
-- collateral referenced no order at all. On the account's real INFY short,
-- summing the rows that reference its orders gave -4708.62 against a true cash
-- effect of -18.19 -- adrift by exactly the 4690.43 that row carried. The long
-- position beside it reconciled to the paisa, which is why every earlier sweep
-- passed: only a short cover writes this row.
--
-- The cash was never wrong; it was unattributable. §12.9 names MARGIN_RELEASE as
-- a row that must be counted, so the contract already intended this and the
-- implementation could not express it. Per CLAUDE.md's resolution order the
-- contract wins, so the function takes the order id.
--
-- The signature change forces a drop -- `create or replace` cannot change an
-- argument list -- and dropping discards the ACL, after which Supabase's
-- defaults re-grant EXECUTE to public, anon and authenticated. The revoke is
-- therefore repeated below, exactly as 20260905... had to. `11-order-identities.sql`
-- asserts which functions a browser can reach and would catch its absence.
--
-- `execute_order` is reproduced whole because a migration cannot patch a
-- function in place, and because its call would otherwise resolve to a
-- three-argument function that no longer exists. The only change to it is that
-- one call site.

drop function if exists public.recompute_position_collateral(uuid, text, integer);

create function public.recompute_position_collateral(
  p_user_id uuid,
  p_symbol text,
...
begin
  ...
  values (p_user_id, 'MARGIN_RELEASE', v_released, v_funds.available_cash, p_order_id,
