-- `execute_order` debited a short entry's charges twice.
--
-- §6 step 6 is part of `transfer_margin_to_position`: it releases the estimated
-- charges from the reservation and debits the actual ones, as a MARGIN_RELEASE
-- and a CHARGES row. `execute_order` then wrote its own CHARGES row for the same
-- fill, so every short entry — and every fill that flipped a long into a short —
-- paid its charges twice and left two CHARGES rows on one order.
--
-- The fix is a condition, not an extra row: the transfer path already accounts
-- for the whole order's charges, because `execute_order` passes it the full
-- figure precisely so a crossing fill still writes exactly one CHARGES row.
--
-- Found by hand-computing the short round trip for `10-orders.sql`, before any
-- assertion ran. §12.6 would not have caught it — the breakdown reconciles fine
-- either way — and identity 1 holds too, because both rows are real cash. Only
-- the arithmetic on the expected balance shows it.
