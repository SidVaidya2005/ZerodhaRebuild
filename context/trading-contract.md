...
- **`recompute_position_collateral(user_id, symbol, new_net_quantity, order_id)`** is the release side of the same formula, called on any fill that *reduces* a short. It takes the order that caused the release and stamps it on the `MARGIN_RELEASE` row, because §12.9 counts that row and cannot be evaluated for a short cover without it. It recomputes `required_collateral` over the quantity the position is **about to become** and returns the difference to `available_cash` with a `MARGIN_RELEASE` row — cash genuinely returns here, unlike on entry, which is why this one writes a row and step 5 does not.
...
- **A fill's closing leg settles before its reservation is retired**, and on a flip that decides the outcome. `transfer_margin_to_position` weighs the shortfall against `available_cash`, so a SELL crossing zero must have its `SELL_CREDIT` posted first or it is judged against a balance excluding the proceeds of the sale being executed — refusing a flip that its own sale pays for. The pair is one subtransaction: a flip that is genuinely short of collateral rolls back whole and writes nothing, rather than stranding a credit for a sale that never happened.
