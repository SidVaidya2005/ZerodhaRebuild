...
**Next:** 16 Market tick Edge Function and schedule — planned 2026-08-21. Gate, demand union, simulator fetch and quote upsert in a Deno function, behind a `pg_cron` job with a Vault-held credential. Built now with the simulator so the schedule, the gateway authentication and the session gate are proven before a real provider exists. Watchlists join the demand union; the candle prune moves to F33; `match_open_orders` and `square_off_mis` are wired in by F28/F29
