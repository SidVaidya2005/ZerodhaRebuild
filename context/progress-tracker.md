...
**In progress:** **The Phase 4 checkpoint, part-run and deliberately not ticked.** Done: tiers 1/2/4 green (381 tier-1, 22/22 pgTAP, 36 parity), §12 identities 1–6, 8, 11 and 12 swept against the live database, the phase diff reviewed by `code-review` over `4e8eb86..0f711d5`, `library-docs.md` re-checked against Context7 (no drift), §3's rate table re-verified against Zerodha with every rate unchanged, 44/44 migrations applied and `database.ts` in sync. **Outstanding, both blocked on the 15:20 sweep:** tier 3's two square-off suites, whose pre-flight correctly refuses to run while real MIS positions are open, and identities 7, 9 and 10, which need closing legs the account does not yet have. Six files uncommitted; tip is `5.31.02`
**Next:** After the 15:20 sweep: re-run `pnpm test:race` on a flat account and re-sweep identities 7, 9 and 10. **Only then** promote what still binds, compact the journal per its own rules, re-run `pnpm context:cost`, and tick the checkpoint box — compacting before those two land would destroy the per-feature detail they are checked against. Then **F33 Stock detail** (`lightweight-charts`, the `candles`/`candle_sync` tables, and F33's retention logic)
