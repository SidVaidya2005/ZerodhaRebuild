...
**Phase:** Phase 6 — Polish & Ship, **F40 next** — README, demo and handoff, the last feature in the project. **F16 and the Phase 2 checkpoint stay open by decision**; F16 lands at the very end. Do not tick either
...
**In progress:** Nothing
**Next:** **F40 README, demo, and handoff**, then the Phase 6 checkpoint. **One F39 item cannot pass yet and is recorded, not hidden:** `cron.job_run_details` showing runs while the web service sleeps needs a `pg_cron` job, which needs **F16**. Nothing schedules quote refreshes or limit-order matching on production today. **Four other follow-ups are open**, all in `phase-6.md`: three terminal tables and the dashboard donut were accessibility-audited as empty states; the destructive button's dark hover is 4.09:1; the keyboard-only order flow and F37's 375×667 submit check both stay deferred; and `trading-contract.md` §13's money sweep has never covered `constraints.md` (6.00.02)
