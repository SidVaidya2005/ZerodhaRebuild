...
**In progress:** Nothing
**Next:** **F38 Accessibility pass.** **One F37 check is deferred:** the order ticket's end-to-end *submit* was never pressed — its geometry, fields, estimate and enabled button are all verified at 375×667, but the final click places a simulated order, so it was left to you. **Before F39 deploys**, either Yahoo lands or `/` and `/about` are reconciled — every price badges `SIMULATED` today. **One finding is open from 6.00.02:** `trading-contract.md` §13's money sweep has never covered `constraints.md`, so `constraints/trading.md`'s charge and margin rationale sits outside it
