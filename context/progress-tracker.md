...
**Last completed:** **F39 Deploy. The app is live at <https://zerodha-rebuild.onrender.com>**, signing in with Google against the one hosted Supabase project. `/api/health` was built from nothing (the directory held only a `.gitkeep`) and reports build and database status through a new `health_check()` `security definer` function — `anon` can read no table here and `constraints/security.md` forbids granting it one, so a function returning a boolean and a timestamp is the narrow exception, and `21-health-check.sql` asserts it widened nothing. Two deploys failed first: `corepack enable` cannot write to Render's read-only `/usr`, and a build carrying `NEXT_PUBLIC_SITE_URL=http://localhost:3000` sent every sign-in to localhost while every other signal stayed green
**In progress:** **F40 README, demo, and handoff.** Planned 2026-09-13; the refined scope is in `phase-6.md`. The demo is the screenshots plus the live URL — **a seeded portfolio is not buildable** (holdings come only from `execute_order`, which needs an open session and a fresh quote) and a *shared* demo login is impossible (Google OAuth is the only sign-in path). The account is reset to its post-signup state instead. Repository links from About and the footer were **already shipped** and need nothing
**Next:** Finish F40, then the Phase 6 checkpoint. **The F39 item previously recorded as unpassable has now passed, and the claim that blocked it was wrong:** `cron.job` holds an active per-minute `market-tick`, `cron.job_run_details` records **7,200 successful runs** (last 2026-09-11 10:59 UTC), and `quotes` stops at 15:29 IST — the session gate refusing to write after the close, which is the behaviour F16 was built to have. The deployed function is **v11 of 2026-09-06**, after F28 and F29 landed, so limit-order matching and MIS square-off are live too; the one later `_shared` change was additive only. **F16 is held open by choice, not by a gap.** **Four follow-ups remain open**, all in `phase-6.md`: three terminal tables and the dashboard donut were accessibility-audited as empty states; the destructive button's dark hover is 4.09:1; the keyboard-only order flow and F37's 375×667 submit check both stay deferred; and `trading-contract.md` §13's money sweep has never covered `constraints.md` (6.00.02)
