...
**Next:** **F33 Stock detail** — `lightweight-charts`, the `candles` / `candle_sync` tables, and the retention logic F15/F16 deferred to it. Read F33's section of `build-plan/phase-5.md` and nothing else of that file. Two things to know first: `phase-4.md` lines 151 and 268 still name `recompute_position_collateral`'s old three-argument signature, left alone because the build plan is not a checkpoint's to edit, and the Phase 2 checkpoint stays open behind F16
