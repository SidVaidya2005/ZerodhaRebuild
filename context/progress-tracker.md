...
**In progress:** 30 Holdings page — three of its four browser items pass (server-rendered provenance, a tick moving row and footer together, Exit prefilled Sell/CNC/qty 11). Two small gaps remain, both needing a **visible** browser window rather than a session: F25's focus-return is inconclusive because a hidden tab never fires `animationend`, and the holdings scroll region was not measured at 375px. F26's market-buy toast is also still open — the seam overrides the *tick's* gate, not `place_order`'s `market_state`
