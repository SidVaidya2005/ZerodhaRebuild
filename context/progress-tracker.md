...
**In progress:** **31 Positions page — built, every non-browser gate green, deliberately not ticked.** `portfolio_positions` and `portfolio_positions_summary` pushed and typed; the page, table, empty state, square-off banner and `positions-totals` all landed. Verified: typecheck, lint, `format:check`, `build`, 362 tier-1 (27 new across `positions-totals` and `square-off`), 21/21 pgTAP files (`16-positions-views.sql` 16/16), 36 parity. The empty state renders correctly and the banner correctly stays silent with no positions. **Three browser items need a live MIS position and so wait on the session**: the short's signed quantity and collateral, server-rendered provenance, and the 375px scroll region
**Next:** **32 Funds page — built, every non-browser gate green, not ticked.** `funds_overview` pushed and typed; cards, ledger table, type filter, pager, `resetAccount` action and `ResetAccountDialog` all landed. Verified live against the real account: cards read ₹76,737.96 / ₹0.00 / ₹1,00,000.00 / −₹10.61, 19 ledger rows, `?type=CHARGES` filters to 4 and filters the count with them, `?type=NOT_A_TYPE&page=abc` falls back to the unfiltered first page with no error boundary, the scroll region is `position: relative` + `tabIndex 0` + labelled with zero sideways scroll, and **exactly one reset control with no forms on the page** — the "no path resets without the dialog" criterion. Outstanding: the dialog's own rendered counts, which need an unoccluded window. **The 09:15 reset now goes through this UI rather than the RPC**, which closes that item and F32's end-to-end criterion together. Then, at 09:15 IST today (Tuesday 2026-09-08, not in `nse-holidays.json`; `pg_cron` job 1 active on `* 3-10 * * 1-5`, last eight runs succeeded): reset the account, which still carries Sunday-dated verification rows (₹76,737.96 cash, 8 orders, 4 trades, TCS ×11); place a CNC market buy for **F26's** toast and its no-reload arrival in Holdings; place an **MIS** buy to open the position F31's remaining browser items need. Then tick F31 and run the Phase 4 checkpoint. No `market_state` stub was ever needed or written; tier 4 proves it
