...
Prices are generated by a simulated tick engine that walks from a real NSE closing price. No external
market-data provider is wired, so every quote badges `SIMULATED`. Every quote carries its provenance,
so the user always knows the provider, how old the figure is, and whether the motion they are watching
is interpolated — the point being that the system cannot overclaim even when it would flatter it.
...
ZerodhaRebuild reproduces those mechanics faithfully against simulated prices with nothing at stake.
...
Realtime for the symbols on screen; a scheduled Edge Function refreshes those quotes from the tick
engine, and between refreshes the client interpolates micro-ticks so the LTP moves and flashes green
or red. A badge on the shell reports the provenance of what is on screen — `SIMULATED` in this build,
since `LIVE` and `DELAYED` are both unreachable without a real provider — and whether the market is
open.
...
- Quote pipeline built as a provider chain with a circuit breaker and per-quote provenance derived at read time. **The simulated tick engine is the whole chain, not its fallback** — no external market-data provider is wired, by decision: Twelve Data's free plan carries no NSE symbols at all, and no other keyless source proved workable. The chain remains a seam a real provider could drop into.
- Cached OHLC candle series for the price chart, fetched on demand per interval with TTLs and daily pruning.
- Demand-driven quote refresh: only symbols currently in view, held, or referenced by an open order are refreshed. The token-bucket rate limiter is a seam and stays unfinished, because a bucket in front of a local simulator caps nothing.
...
- Every displayed price resolves accurate provenance — provider, provider timestamp, and whether the figure is interpolated — and the derived badge reads SIMULATED rather than overclaiming. `LIVE` and `DELAYED` are both unreachable in this build and the public copy says so.
