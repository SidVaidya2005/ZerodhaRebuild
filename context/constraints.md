...
- **Every function that locks `funds` and `positions` takes them in that order.** `execute_order` does, so `square_off_mis` holding a position row across its `execute_order` call inverted the pair and deadlocked with any concurrent order on the same user and symbol. **Adding a guard under a row lock is also a change to lock order** — check it against every other holder of those rows. (F29)
- **A `raise warning` + counter exception handler can make a concurrency bug look like a clean run.** `square_off_mis` swallows a deadlock (`40P01`) as a "fault", so removing its position lock left the end state identical and every end-state assertion green. Assert what a sweep returns, not only what it left behind. (F29)
