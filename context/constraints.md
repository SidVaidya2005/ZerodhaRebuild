...
- **An `sr-only` element inside a wide table escapes its `overflow-x-auto` region and makes the whole page scroll sideways.** `.sr-only` is `position: absolute`, so with neither the `<th>` nor the scroll wrapper positioned it resolves against the initial containing block and is never clipped. **Every scroll wrapper now carries `relative`** — Holdings, Orders, Positions, Ledger and dashboard RecentOrders — so any new table must too. (F27, F30, fixed across all five 2026-09-09)
