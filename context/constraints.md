...
- **One pre-checkpoint `MARGIN_RELEASE` row carries no `order_id`**, written before `recompute_position_collateral` took one. §12.9 is unevaluable for that one cover and holds for every cover since; it was left rather than backfilled, because rewriting a ledger row to make an identity pass is the failure the ledger exists to prevent. Cleared by the next §11 reset. (Phase 4 checkpoint)
...
- **`modify_order` reuses `reserve_margin` inside a plpgsql subtransaction rather than recomputing the requirement.** `release_margin` → new terms → `reserve_margin`, in a block with an `EXCEPTION` clause, so a failed re-reservation rolls back only that block and returns `(false,'INSUFFICIENT_FUNDS')` with the order on its original terms and reservation. Recomputing in place would be a second copy of §6, and the release must come first because `reserve_margin` returns early when `blocked_margin <> 0`. (F27)
...
- **The Orders page filters on `status=OPEN`, `placed_at` today, *or* `executed_at` today.** The first two alone lose an order at the moment it stops being open — one placed before today that fills, cancels or rejects today matches neither, and it vanished from the page rather than moving to Executed. `executed_at` is stamped on every terminal transition, so the third condition covers all three. (F27, fixed at the Phase 4 checkpoint)

## Testing

- **Tier 4's margin parity compares against a TypeScript re-implementation of `reserve_margin`'s branch selection, not against `reserve_margin`.** It sends only the arithmetic to Postgres, so a change to *which* branch nets against the position would leave the ticket wrong with parity still green. Tier 2's `08-margin.sql` covers the function directly; the gap is the cross-language branch comparison. **Not fixed** (Phase 4 checkpoint).
...
- **Check what tier 2 already proves before writing new assertions.** F31's exit and F32's reset were each already covered by `10-orders.sql` — the long close, the short cover and §12.5 deletion for one, section L's six §11 assertions for the other — so both features cite that coverage rather than copy it. A second copy of a passing assertion is not coverage, and the two drift. (F31, F32)
...
- **`market-tick` takes a `session_at` override in its request body**, read *after* the secret check so it is exactly as restricted as the Vault credential. It overrides the session gate and the square-off boundary only — `now` still stamps every recorded timestamp, and the response carries `sessionOverride: true` so an overridden run is never mistaken for a real one. It does not open `place_order`, whose gate is Postgres `market_state`. (Phase 4 checkpoint)
...
- **The pinned lock order is per user, not across users.** `match_open_orders` sweeps by `placed_at` and `square_off_mis` by `opened_at`, each holding one owner's `funds` row for the rest of its run, so two overlapping sweeps can close an ABBA cycle across *different* users. `15-lock-order.sql` reads the sequence out of each definition and structurally cannot see it. **Not fixed** (Phase 4 checkpoint).
...
**Moved to `context/constraints/verification.md`** — read it before driving the app in a browser
or trusting an automated check. Six features have lost time to those traps, but none of them binds
a session that never opens a browser, so it is on demand rather than always-read.
...
- **An `sr-only` element inside a wide table escapes its `overflow-x-auto` region and scrolls the whole page sideways.** `.sr-only` is `position: absolute`, so without a positioned ancestor it resolves against the initial containing block and is never clipped. **Every scroll wrapper carries `relative`**; any new table must too. (F27, F30)
...
- **Below 1024px the terminal has no page navigation at all, and F37 owns the fix.** `TERMINAL_NAV_LINKS` is `hidden lg:flex` while the sheet trigger is `md:hidden`, so 768–1024px has neither; the sheet itself carries only `/stocks/*`, never the six routes, despite `nav-links.ts` claiming it consumes them. `terminal-routes.test.ts` proves each link is guarded and has a page, never that anyone can reach it. (F31, confirmed 2026-09-09)
...
- **Two identical GET reads in one render return the same answer, even across a write between them.** Next memoizes identical `fetch` calls per render pass and supabase-js is built on `fetch`, so `read → upsert → read` replayed the pre-write result and the chart drew empty over 276 rows just written. **`cache: 'no-store'` does not defeat it** — memoization is not the Data Cache. Have the writer return what it wrote instead of re-reading. (F33)
- **PostgREST answers `PGRST103` for an offset past the end of a set, returning null rows *and* a null count.** A pager whose total comes from `{ count: 'exact' }` therefore loses that total exactly when the page is out of range, and renders "no rows" over an account that has them — take the total from a Postgres aggregate and keep the count as a cross-check. (F34)
