...
- **Assert sideways scroll on the page, never on the region.** A region can be focusable, labelled and `relative` while the page still overflows: `/positions` overflowed 94px on the shared `TopNav` alone, and `/holdings` 525px on a wrapper the constraint above had already mandated. Measure `documentElement.scrollWidth` vs `clientWidth` for **each page**, and re-measure every page after touching shared chrome. (F31)
