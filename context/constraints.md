...
- **An `sr-only` element inside a wide table escapes its `overflow-x-auto` region and makes the whole page scroll sideways.** `.sr-only` is `position: absolute`, so with neither the `<th>` nor the scroll wrapper positioned it resolves against the initial containing block and is never clipped — measured at 365px of page scroll on `/orders` and 521px on `/holdings`, both with otherwise-correct regions. Give it a positioned ancestor; applies to every table F31, F33 and F34 add. (F27, F30, measured at the Phase 4 checkpoint)
