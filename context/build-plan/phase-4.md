...
- Test (tier 3, `squareoff.race.test.ts`): two simultaneous runs exit a position exactly once. **Falsifiable — apply `20260905150000_square_off_mis.sql` (the pre-fix body) and it goes red on B's `faulted` counter.** Confirmed 2026-09-05, three runs each way, and six consecutive clean runs against the real build. The end state is *identical* on both builds — same position count, same trade count, same ledger identity — so what carries this test is that the two sweeps return `(1, 0)` and `(0, 0)`. Without the re-read, B is already inside `execute_order` holding the `funds` row lock when it blocks on the position row; A then wants that same row, and `exception when others` swallows the resulting deadlock as a fault. A deadlock, not the guard, is what kept the end state clean, which is why the first version of this test passed against both builds.
- The intermittent timeout was `waitUntilBlocked` identifying the blocked backend by `pg_stat_activity.query`, which is not dependable through Supavisor. It polls `pg_blocking_pids()` on B's own pid now, and the pre-flight check runs before seeding across every symbol so a stray fixture cannot make the sweep multi-row.
- **Follow-up, undecided:** `square_off_mis`'s `exception when others` swallows `40P01` and `40001` alongside a genuine row fault, which F29 demonstrated rather than merely suspected. It self-heals — the job retries every minute until 15:30 — but a concurrency abort and a bad row are counted identically and nothing reads `faulted`. Decide whether to separate or re-raise them before Phase 5 leans on the counter.
