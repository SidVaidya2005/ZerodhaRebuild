...
- `scripts/capture-screenshots.mts` — **eight** shots regenerated by one command, into
  ...
  it**, so it currently uses five and grows to nine.
- **Follow-up, carried deliberately: re-run the capture during a trading session** (Mon–Fri
...
- The account is at its post-signup state: `available_cash = 100000.00`, `used_margin = 0`,
  ...
  `positions`.
- `pnpm capture:screenshots` writes four files into `docs/screenshots/` and is idempotent on a
