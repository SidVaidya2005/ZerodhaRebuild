...
- All market-hours and square-off logic computes in `Asia/Kolkata` regardless of server timezone; no code calls `new Date()` for market logic without going through `src/lib/market/market-hours.ts`.
