...
> ⚠️ **REFERENCE PRIMITIVES ONLY.** This document describes Binance's interface. Take from it the
> colour, type, spacing, elevation and shape *primitives* — nothing else. **Never implement a component,
> product surface, or line of copy described here**: no crypto or coin UI, no deposit/withdraw or buy-crypto
> flows, no Smart Money surfaces, no markets/pairs tables, no Binance wordmark, naming, claims or
> illustrations. This project trades NSE equities with simulated money and has no such surfaces. When a
> section below describes a component, read it for its anatomy — sizes, radii, borders, states — and build
> ZerodhaRebuild's own component with ZerodhaRebuild's own copy.
>
> **Role:** The visual system this project's design tokens are derived from — colour, type, spacing, elevation, shape, and component anatomy.
> **Read before building any UI.** The `@theme` token set that implements it lives in `library-docs.md` → Tailwind CSS v4.
> **Relates to:** three deliberate deviations for this project — own branding, substituted fonts, dark-by-default on both surfaces — are documented in `library-docs.md` → "Adapting the system to this project". Where this file and those tokens disagree, the tokens win, because they are what ships.
