...
/**
 * The prices tile says "simulated", not "live" and not "real". No provider in
 * this build streams ticks, and as shipped **none is wired at all**: the chain
 * falls straight through to the simulator, so every quote badges SIMULATED.
 * Saying "real market data provider" here was true of the design and false of
 * the build — see architecture.md → Quote Provenance, and the honesty band
 * further down the page, which explains the whole vocabulary. Restore the older
 * wording only when a real provider is actually in the chain.
 */
const FEATURES: readonly Feature[] = [
  {
    icon: LineChart,
    title: 'Simulated prices, labelled as such',
    body: 'Every price here is generated by a tick engine seeded from a real NSE closing price, refreshed on a schedule and interpolated between refreshes so the tape moves. Nothing claims to be a live quote: each price carries a badge saying where it came from and how old it is, and in this build that badge always reads SIMULATED.',
  },
  {
    icon: Receipt,
